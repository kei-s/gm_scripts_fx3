...
var SCRIPT_VERSION  = "2007.11.28"
var SCRIPT_URL      = "http://userscripts.org/scripts/show/11562"
...
// height
// ------------------------------------------------------------------
var SITEINFO_URLS   = ["http://white.s151.xrea.com/wiki/index.php?cmd=ldrize"]
...
// ------------------------------------------------------------------
var IFRAME_SCROLL = 400

// ------------------------------------------------------------------
// not open by iframe
// ------------------------------------------------------------------
var IFRAME_IGNORE = [/\.pdf$/, /\.mp3$/, /\.wmv$/]

// ------------------------------------------------------------------
// image
// ------------------------------------------------------------------
var IMAGE_INDICATOR = 'data:image/png;base64,'
var IMAGE_UP        = 'data:image/png;base64,'
...
// CSS
// ------------------------------------------------------------------
var CSS_STRIPE_ODD       = false
var CSS_STRIPE_EVEN      = 'background-color:#eeeeff !important;'
var CSS_HIGHLIGHT_LINK   = false
var CSS_HIGHLIGHT_PINNED = '-moz-outline: 2px solid #CC6060 !important;-moz-outline-offset: 1px !important;-moz-outline-radius: 3px !important;'
...
LDRize.prototype = {
  ...
  initMinibuffer: function(){
	  ...
	  lst.forEach(function(arg){window.Minibuffer.addCommand(arg)});
  },
...
// pin
  ...
  addPinToPinList: function(node){
	  ...
	  if(!paragraph.html) {
		  ...
		  var view = this.getSiteinfo()['view'];
		  var xpath = view || '(descendant-or-self::img | descendant::text()[normalize-space(self::text()) != ""])';
		  var matches = $X(xpath, node);
		  ...
		  if(view){
			  ...
			  for(var i=0; i<matches.length; i++){
				  if(matches[i].nodeName.toLowerCase() == 'img' || matches[i].nodeType == 3) {
					  var h = new Paragraph(matches[i]).y;
					  if(Math.abs(height - h) >= 20) break;
					  if(matches[i].nodeType == 3){
						  var str = matches[i].nodeValue.replace(/\s/g,'');
						  allstringlength += str.length;
						  if(allstringlength > 30){
							  res.push(str.slice(0, 30) + '...');
							  break;
						  }else{
							  res.push(matches[i].nodeValue);
						  }
					  }else{
						  res.push(getCloneImage(matches[i]));
					  }
				  }
			  }
		  }
  ...
  getPinnedItemsOrCurrentItem: function(){return this.pinIsEmpty() ? [this.getParagraphes().current.paragraph] : this.getPinnedItems()},
...
// i -- view in iframe
  bindIframe: function(){
	  ...
	  if(IFRAME_IGNORE.some(function(re){return url.match(re)})) return;
  ...
  getCurrentLink: function(){
	  ...
	  return xpath ? [this.getParagraphes().current.paragraph.XPath(xpath)] : false;
  },
...
Paragraphes.prototype = {
  ...
  collect: function(){
	  ...
	  for(var i=0,l=matches.length; i<l; i++){
		  ...
		  if(this.context.length && !this.context.some(function(e){return descendantOrSelf(node, e)})) continue;
  ...
  currentIsValid: function(){
	  return this.current && (this.current.position >= 0) && (this.current.position < this.length);
  },
...
SiteinfoOperator.prototype = {
  ...
  initializerCaller: function(){
	  ...
	  if(++this.counter == this.urls.length && this.siteinfo.length) {
		  this.initializer(this.siteinfo.map(function(arg){return new Siteinfo(arg)}));
	  }
  },
  init: function(){
...
//// LivedoorReader FastLadder only
...
if(document.body) {
	...
	var parser = function(res){
		var trim = function(t){return t.replace(/^\s+/,'').replace(/,$/,'').replace(/^\'/,'').replace(/\'$/,'');}
		var content=new RegExp("^ ([^:]+):(.+)"), sep=new RegExp("^[^ ].+") ,comment=new RegExp("^//");
		var lines=res.responseText.split("\n"), res=[], current_info={};
		for(var i=0; i<lines.length; i++){
			var line = lines[i];
			if(line.match(comment)){
				continue;
			}else if(line.match(sep)){
				current_info = {};
				res.push(current_info);
			}else if(line.match(content)){
				var property = RegExp.$1;
				var val = trim(RegExp.$2).replace(/\\\\/g,'\\');
				if(val.toLowerCase() == 'true' || val.toLowerCase() == 'false') val = eval(val);
				current_info[property] = val;
			}
		}
		return res;
	}
